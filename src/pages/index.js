...
export default function Browse() {
    ...
    async function getMatch(token) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API}/api/v1/matches`, {
          headers: {
    ...
    return (
        ...
        <Confirmation onCancel={handleCancel} user={user} type={type}/>
      ...
      match.map((item, index) => {
        return(
          ...
          <div key={index} className={`border border-gray-125 p-4 m-4 rounded-md mt-16 `}>
          {/* Platform Details */}
          ...
          <div className="flex items-center">
              ...
              <div className="mt-4">
                  ...
                  <div className="bg-white rounded-md h-8 relative px-2 flex items-center">
                      <div className={`h-6 bg-[#6590FD] rounded-md ml-0 flex-shrink-0`} style={{width:`calc(${((Number(item.under) / Number(item.total)) * 100).toFixed(2)}% - 2px)`}}></div>
                      <div className="flex-grow"></div>
                      <div className={`h-6 bg-[#FF6E1B] rounded-md mr-0 flex-shrink-0 `} style={{width:`calc(${((Number(item.over) / Number(item.total)) * 100).toFixed(2)}% - 2px)`}}></div>
                  </div>
                  <div className="flex justify-between mt-2">
                      <span>
                          <span className="font-g8-semibold text-[12px] text-[#9E9E9E]" >Under: </span>
                          <span className="font-g8-extrabold text-[12px] text-[#6590FD]" >{((Number(item.under) / Number(item.total)) * 100).toFixed(2)}%</span>
                      </span>
                      <span>
                          <span className="font-g8-semibold text-[12px] text-[#9E9E9E]">Over: </span>
                          <span className="font-g8-extrabold text-[12px] text-[#FF6E1B]">{((Number(item.over) / Number(item.total)) * 100).toFixed(2)}</span>
                      </span>
                  </div>
              </div>
          </div>
